...
struct C620 {
    ...
    void set_current(float current) { raw_current_ = current * max; }
    uint16_t get_angle() { return rx_.angle; }
    int16_t get_rpm() { return rx_.rpm; }
    int16_t get_ampere() { return rx_.ampere; }
    uint8_t get_temp() { return rx_.temp; }
